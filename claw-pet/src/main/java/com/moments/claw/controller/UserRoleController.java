...
/**
 * 用户、角色关联表(ClawUserRole)表控制层
 *
 * @author chandler
 * @since 2024-04-20 21:12:28
 */
@Api(tags = "ClawUserRoleController控制层", value = "/clawUserRole")
@RestController
...
public class UserRoleController extends BaseController {
    ...
    public TableDataInfo<?> selectAll() {
        startPage();
        return getDataTable(userRoleService.list());
    }
    ...
    public R<?> selectOne(@ApiParam(name = "id", value = "id", required = true) @PathVariable Serializable id) {
        return R.success(userRoleService.getById(id));
    }
    ...
    public R<?> delete(@ApiParam(name = "idList", value = "id数组", required = true) @RequestParam("idList") List<Long> idList) {
        return R.success(userRoleService.removeByIds(idList));
    }
}
